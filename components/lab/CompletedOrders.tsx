...
export default function CompletedOrders() {
  ...
  useEffect(() => {
    ...
    }, searchTerm || priorityFilter !== 'all' ? 500 : 0);
  ...
  const downloadReport = async (orderId: string, orderNumber: string) => {
    try {
      ...
      const reportHeaderHTML = `
        ...
        <div style="text-align: center; margin-bottom: 16px; padding-bottom: 12px;"><div style="display: flex; justify-content: center; margin-bottom: 8px; width: 100%;"><img src="/logo.jpeg" alt="Health Inn Services" style="max-width: 180px; height: auto;" width="180" height="70"></div></div>
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        testResultHTML += `
          <h4 style="font-weight: bold; color: #111; margin-bottom: 16px; text-align: center; font-size: 14px;">${result.test?.name || 'TEST RESULTS'} (${result.test?.code || 'N/A'})</h4>
            ...
            <table style="width: 100%; font-size: 12px; border-collapse: collapse;">
              ...
              <tbody>
                ${result.resultData && result.resultData.length > 0 ?
                  result.resultData.map((param: { parameter?: string; value?: string; unit?: string; normalRange?: string; }) => `<tr style="border-bottom: 1px solid #ccc;"><td style="border: 1px solid #ccc; padding: 8px; font-size: 12px;">${param.parameter || 'N/A'}</td><td style="border: 1px solid #ccc; padding: 8px; font-size: 12px;">${param.value || 'N/A'}</td><td style="border: 1px solid #ccc; padding: 8px; font-size: 12px;">${param.unit || '-'}</td><td style="border: 1px solid #ccc; padding: 8px; font-size: 12px;">${param.normalRange || '-'}</td></tr>`).join('')
              ...
              <div className="bg-white border rounded-lg p-8" style={{ fontFamily: 'Arial, sans-serif' }}>
                ...
                <div className="text-center mb-4 pb-3">
                  <div className="flex justify-center mb-2 w-full">
                    <Image src="/logo.jpeg" alt="Health Inn Services" className="object-contain" style={{ maxWidth: '180px', height: 'auto' }} width={180} height={70} />
                  </div>
                </div>
                  ...
                  {viewingResults && viewingResults.length > 0 ? (
                    viewingResults.map((result) => (
                      <div key={result._id} className="mb-8">
                        <h3 className="font-bold text-gray-900 mb-2 text-center text-sm uppercase">{result.test?.name || 'TEST RESULTS'}</h3>
