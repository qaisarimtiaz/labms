...
export default function PatientManagement() {
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    e.preventDefault();
    if (creating) return; // guard against double-submit (double-click, Enter key repeat, etc.)
    setCreateError('');
    setCreating(true);
        ...
        <form onSubmit={handleCreateUser}>
          ...
          <ModalFooter>
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-accent rounded-[var(--radius-md)] transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={creating}
              className="inline-flex items-center px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground rounded-[var(--radius-md)] text-sm font-medium shadow-[var(--shadow-sm)] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {creating ? (
                <>
